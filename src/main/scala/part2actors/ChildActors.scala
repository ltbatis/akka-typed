  ...
  object Parent {
    ...
    def active(childRef: ActorRef[String]): Behavior[Command] = Behaviors
      ...
      .receiveSignal { case (context, Terminated(childRefWhichDied)) =>
        context.log.info(
          s"[parent] Child ${childRefWhichDied.path} was killed by something..."
        )
        idle()
      }
  }
        ...
        .receive[Command] { (context, message) =>
          message match {
            ...
            case TellChild(name, message) =>
              ...
              childOption.fold(
                context.log.info(s"[parent] Child '$name' could not be found")
              )(child => child ! message)
              Behaviors.same
            case StopChild(name) =>
              context.log.info(
                s"[parent] attempting to stop child with name $name"
              )
              val childOption = children.get(name)
              childOption.fold(
                context.log.info(
                  s"[parent] Child $name could not be stopped: name doesn't exist"
                )
              )(context.stop)
              active(children - name)
            case WatchChild(name) =>
              context.log.info(
                s"[parent] watching child actor with the name $name"
              )
              val childOption = children.get(name)
              childOption.fold(
                context.log
                  .info(s"[parent] Cannot watch $name: name doesn't exist")
              )(context.watch)
              Behaviors.same
          }
        }
