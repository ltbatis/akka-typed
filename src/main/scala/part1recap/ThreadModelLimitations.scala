...
object ThreadModelLimitations {
  ...
  val withdrawThreads = (1 to 1000).map(- => new Thread(() => account.withdraw(1)))
  ...
  */
  ...
  // DR #3 - tracing and dealing with erros is a PITN in multithreaded/distributed apps

  implicit val ex: ExecutionContext = ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(8))
  // sum 1M number in between 10 threads
